...
class HomeWork8  extends JFrame {
    ...
    final int MAX_COLOR = Field.getMAX_COLOR();
    final Color[] arrColors = Field.getArrColors();

    private JToggleButton[] btn_color = new JToggleButton[MAX_COLOR];
    ...
    void updateFacade(){
        for (int i=0; i< MAX_COLOR; i++){
            btn_color[i].setEnabled(status>1);
        }
        bg.clearSelection();
    }
